...
function setLevel(newLevel) {
    ...
    bonus.style.display = "none"; // Bonus erst ab Punkten
    bomb.style.display = "none";  // wird je nach Level gesetzt
    ...
    if (level === 1) {
        ...
        timeLeft = 60;
        autoMove = setInterval(moveObjects, 1000); // am schnellsten
    }
...
// --------------------
// 14) Bonus klicken = doppelte Punkte 💎✨
// --------------------
bonus.onclick = () => {
    if (!gameRunning) return;

    score += 2; // 🔥 doppelte Punkte
    scoreText.textContent = score;

    playSound(bonusSound); // Bonus-Sound

    bonus.style.display = "none"; // Bonus verschwindet
...
const timer = setInterval(() => {
   ...
   if (timeLeft <= 0) {
        ...
        if (level === 1 && score >= 20) {
            ...
            "💔 Game Over! Du hast verloren 😢 Punkte: " + score;
            playSound(gameOverSound);
        }
